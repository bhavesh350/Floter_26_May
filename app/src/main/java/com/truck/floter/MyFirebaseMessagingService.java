...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if (dataMap.containsKey("trip_status")) {
            String tripStatus = (String) dataMap.get("trip_status");
            String tripId = (String) dataMap.get("trip_id");
            if (tripStatus.equals(TripStatus.Accepted.name())) {
                MyApp.setStatus("IS_ON_TRIP", true);
                if (remoteMessage.getData().size() > 0) {
                    Log.d(TAG, "Message data payload: " + remoteMessage.getData());
                    Intent i = new Intent("com.truck.floter.NEW_RIDE");
                    i.putExtra("TYPE", "TRIP_ACCEPTED");
                    sendBroadcast(i);
                    sendNotificationAndStartTrip("Driver is on the way", Integer.parseInt((String) dataMap.get("trip_id")));
                }
